        ...
        private void loginbox_Click(object sender, EventArgs e)
        {
            ...
            if (isUser == 1)
            {
                if (isUserLogin == true)
                {
                    ...
                    result = users.Data_deal(id.Text.Trim());
                    MainUsers.name = result.Tables["tables"].Rows[0]["Uname"].ToString();
                    form2.Show();
                    this.Hide();
                }
                else
            ...
            else
            {
                ...
                {
                    Book_manage bookmanager = new Book_manage();
                    Book_manage.currentUser = id.Text.Trim();
                    bookmanager.Show();
                    this.Hide();
                }
                else if(isAdminLogin == (char)3)
                {
                    User_manage usermanager = new User_manage();
                    User_manage.id = id.Text.Trim();
                    usermanager.Show();
                    this.Hide();
                }
                else
